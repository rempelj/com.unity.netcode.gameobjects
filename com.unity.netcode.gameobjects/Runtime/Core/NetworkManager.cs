...
#pragma warning restore IDE1006 // restore naming rule violation check
        ...
        private class NetworkManagerHooks : INetworkHooks
        {
            ...
            {
                if (m_NetworkManager.IsHostlessPeer)
                {
                    return true;
                }
        ...
        /// <summary>
        /// Gets if we are running a hostless peer to peer game (public lobby)
        /// </summary>
        public bool IsHostlessPeer  { get; internal set; }

        /// <summary>
        /// GUID generated by each client to identify a peer when there is no "host"
        /// </summary>
        public ulong LocalHostlessPeerId  { get; internal set; }
        ...
        public bool StartClient()
        {
            ...
            IsClient = true;
            IsListening = true;
            IsHostlessPeer = false;

            return result;
        }

        /// <summary>
        /// Starts a Hostless peer to peer game
        /// </summary>
        public bool StartHostless()
        {
            NetworkLog.LogInfo("StartHostless");
            IsHostlessPeer = true;
            byte[] gb = Guid.NewGuid().ToByteArray();
            LocalHostlessPeerId = BitConverter.ToUInt64(gb,0);
            ...
            if (NetworkConfig.ConnectionApproval)
            {
                InvokeConnectionApproval(NetworkConfig.ConnectionData, ServerClientId,
                    (createPlayerObject, playerPrefabHash, approved, position, rotation) =>
                    {
                        ...
                        HandleApproval(ServerClientId, createPlayerObject, playerPrefabHash, true, position, rotation, LocalHostlessPeerId);
                    });
            }
            else
            {
                HandleApproval(ServerClientId, NetworkConfig.PlayerPrefab != null, null, true, null, null, LocalHostlessPeerId);
            }
        ...
        private void OnNetworkManagerTick()
        {
            ...
            if (!IsHostlessPeer && IsServer && NetworkTickSystem.ServerTime.Tick % timeSyncFrequencyTicks == 0)
            {
                SyncTime();
            }
        }
        ...
        private void SendConnectionRequest(ulong clientId)
        {
            ...
            {
                HostlessPeerId = LocalHostlessPeerId,
            ...
            SendMessage(message, NetworkDelivery.ReliableSequenced, clientId);
        }
            ...
            switch (networkEvent)
            {
                case NetworkEvent.Connect:
                    ...
                    else
                    {
                        ...
                        SendConnectionRequest(ServerClientId);
                        StartCoroutine(ApprovalTimeout(clientId));
                    }
        ...
        internal void HandleApproval(ulong ownerClientId, bool createPlayerObject, uint? playerPrefabHash, bool approved, Vector3? position, Quaternion? rotation, ulong hostlessPeerId = 0)
        {
